...
import {
  FaArrowLeft, FaMapMarkerAlt, FaPhone, FaUser, FaCheckCircle,
  FaStore, FaShoppingCart, FaRupeeSign, FaTimes, FaWalking,
  FaRoute, FaChevronRight,
} from "react-icons/fa";
...
const FORM_ICONS: Record<string, string> = {
  ...
  "Eye Drops": "👁️", "Ear Drops": "👂", "Nasal Spray": "💨", Inhaler: "🫁",
  Injection: "💉", Powder: "🥛", Solution: "🧪", Suspension: "🧪",
  Ointment: "🧴", Drops: "💧", Patch: "🩹", Spray: "💨",
};
...
function buildVisitPlan(cartItems: CartItem[], shops: Shop[]): {
  stops: ShopStop[];
  ...
  const unavailable: CartItem[] = cartItems.filter(
    (ci) => !shops.some((s) =>
      s.medicines.some((sm) => sm.medicine_id === ci.medicine.id && sm.quantity > 0)
    )
  );
  ...
  while (remaining.length > 0) {
    ...
    for (const shop of shops) {
      const covered = remaining.filter((ci) =>
        shop.medicines.some((sm) => sm.medicine_id === ci.medicine.id && sm.quantity > 0)
      );
      ...
      const isBetter =
        ...
        (covered.length === bestCovered.length &&
          parseKm(shop.distance_from_user) < parseKm(bestShop?.distance_from_user ?? "9999 km"));
        ...
        <div className="px-6 pt-6 pb-4 border-b border-gray-100">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <span className="text-2xl">{FORM_ICONS[item.medicine.form] || "💊"}</span>
              <div>
                <p className="text-sm font-bold text-gray-900">{item.medicine.name}</p>
                <p className="text-xs text-gray-400">{item.medicine.brand} · qty ×{item.quantity}</p>
              </div>
            </div>
          ...
          <p className="text-xs text-gray-500">
            Collected from <span className="font-semibold text-gray-700">{shopName}</span>
            {listedPrice > 0 && <span className="text-emerald-600"> · listed ₹{listedPrice}/unit</span>}
          </p>
        </div>
        ...
        <div className="px-6 py-5">
          ...
          {num > 0 && (
            <p className="text-xs text-gray-400 mb-4">
              Total: <span className="font-black text-emerald-600 text-sm">₹{num * item.quantity}</span> for ×{item.quantity}
            </p>
          )}
          <button
            disabled={num <= 0}
            ...
            style={{ background: num > 0 ? "linear-gradient(135deg,#10b981,#0d9488)" : "#d1d5db" }}
          >
            ✓ Mark as Collected
          </button>
        </div>
...
function FindPharmaciesContent() {
  ...
  const [modal, setModal] = useState<{ item: CartItem; shop: Shop; listedPrice: number } | null>(null);
  ...
  useEffect(() => {
    ...
    } catch { setError("Could not read cart."); }
  }, []);
  ...
  // ── Build visit plan ───────────────────────────────────────────────────────
  const { stops, unavailable } = cartItems.length && shops.length
  ...
  const totalPaid = Object.values(received).reduce((s, r) => s + r.pricePaid * r.quantity, 0);
  const progress = totalAvailable > 0 ? (receivedCount / totalAvailable) * 100 : 0;
  ...
  const complete = async () => {
    ...
    const shopSummary: Record<string, {
      shopId: string; shopName: string;
      shopLocation: string; shopPhone: string; subtotal: number;
    }> = {};
    items.forEach((it) => {
      ...
      if (!shopSummary[it.shopId]) {
        shopSummary[it.shopId] = {
          ...
          shopLocation: sh?.location ?? "", shopPhone: sh?.phone ?? "", subtotal: 0,
        };
      }
    ...
    try {
      const prev = JSON.parse(localStorage.getItem("medley_purchase_history") ?? "[]");
      localStorage.setItem("medley_purchase_history", JSON.stringify([record, ...prev]));
  ...
  // ── Screens ────────────────────────────────────────────────────────────────
  if (loading) return (
    <Screen><Spinner color="success" size="lg" /><p className="mt-3 text-sm text-gray-400">Finding pharmacies…</p></Screen>
  );
  ...
  if (error) return (
    <Screen>
      ...
      <button onClick={() => window.location.reload()} className="px-5 py-2 bg-emerald-500 text-white rounded-xl text-sm font-bold">Retry</button>
    </Screen>
  );

  if (done) return (
    <Screen>
      <div className="w-20 h-20 bg-emerald-100 rounded-full flex items-center justify-center mb-4" style={{ boxShadow: "0 0 0 12px #d1fae5" }}>
        <FaCheckCircle className="w-10 h-10 text-emerald-500" />
      </div>
      <h2 className="text-xl font-black text-gray-900 mb-1">All Done!</h2>
      <p className="text-sm text-gray-500">Total paid: <span className="font-black text-emerald-600">₹{totalPaid}</span></p>
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-2xl mx-auto px-4 py-3.5 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-black text-gray-900">Your Medicine Run</h1>
            <p className="text-[11px] text-gray-400 mt-0.5">
              {stops.length} shop{stops.length !== 1 ? "s" : ""} to visit · {cartItems.length} medicine{cartItems.length !== 1 ? "s" : ""}
            </p>
          </div>
          {receivedCount > 0 && (
            <div className="text-right">
              <p className="text-xs font-black text-emerald-600">₹{totalPaid}</p>
              <p className="text-[10px] text-gray-400">{receivedCount}/{totalAvailable}</p>
            </div>
          )}
        </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-5 pb-12">
        ...
        {cartItems.length === 0 ? (
          <div className="text-center py-24">
            ...
            <Link href="/customer" className="text-sm text-emerald-600 hover:underline">← Browse medicines</Link>
          </div>
        ) : (
          <>
            {/* ═══ VISIT PLAN ═══════════════════════════════════════════════ */}
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-xs font-black text-gray-400 uppercase tracking-widest">Your Visit Plan</h2>
              </div>

              {stops.length === 0 ? (
                <div className="bg-white rounded-2xl p-5 text-center border border-red-100">
                  <p className="text-sm font-bold text-red-400">No nearby pharmacy has your medicines in stock</p>
                </div>
              ) : (
                <div className="relative">
                  ...
                  <div className="space-y-3 relative z-10">
                    {stops.map((stop, stopIdx) => {
                      const stopDone = stop.medicines.every((m) => received[m.medicine.id]);
                      const stopPartial = stop.medicines.some((m) => received[m.medicine.id]);
                          ...
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                              <p className="text-xs font-black text-gray-700 uppercase tracking-wide">
                                {stopIdx === 0 ? "Go here first" : stopIdx === 1 ? "Then go here" : `Stop ${stopIdx + 1}`}
                              </p>
                            </div>
                          </div>
                          ...
                          >
                            ...
                            <div className="px-4 py-3.5">
                              <div className="flex items-start justify-between gap-2">
                                <div className="flex items-center gap-2.5">
                                  ...
                                  <div>
                                    <p className="text-sm font-black text-gray-900">{stop.shop.name}</p>
                                    <div className="flex flex-wrap gap-x-2.5 gap-y-0 mt-0.5">
                                      <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-600">
                                        <FaWalking className="w-2.5 h-2.5" />{stop.shop.distance_from_user}
                                      </span>
                                      <span className="inline-flex items-center gap-1 text-[11px] text-gray-500">
                                        <FaMapMarkerAlt className="w-2.5 h-2.5 text-gray-400" />{stop.shop.location}
                                      </span>
                                    </div>
                                  </div>
                                </div>
                                <div className="text-right flex-shrink-0">
                                  <span
                                    ...
                                    style={{
                                      background: stopDone ? "#d1fae5" : "#ede9fe",
                              ...
                              <div className="flex flex-wrap gap-x-3 gap-y-0 mt-2">
                                <span className="inline-flex items-center gap-1 text-[11px] text-gray-400">
                                  <FaPhone className="w-2.5 h-2.5" />{stop.shop.phone}
                                </span>
                                <span className="inline-flex items-center gap-1 text-[11px] text-gray-400">
                                  <FaUser className="w-2.5 h-2.5" />{stop.shop.owner}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="border-t border-gray-100">
                              {stop.medicines.map((item, mIdx) => {
                                ...
                                const isLast = mIdx === stop.medicines.length - 1;

                                return (
                                  <div
                                    key={item.medicine.id}
                                    className={`flex items-center gap-3 px-4 py-3 ${!isLast ? "border-b border-gray-50" : ""}`}
                                    style={{ background: rec ? "rgba(16,185,129,0.04)" : "transparent" }}
                                  >
                                    {/* Pill icon */}
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-center gap-1.5 flex-wrap">
                                        <span
                                          ...
                                          style={{ textDecoration: rec ? "line-through" : "none", opacity: rec ? 0.5 : 1 }}
                                        >
                                          {item.medicine.name}
                                        </span>
                                        <span className="text-[10px] text-gray-400">×{item.quantity}</span>
                                      </div>
                                      <p className="text-[11px] text-gray-400">{item.medicine.brand} · {item.medicine.form}</p>
                                      {rec ? (
                                        <p className="text-[11px] text-emerald-600 font-semibold mt-0.5">
                                          Collected · ₹{rec.pricePaid}/unit · ₹{rec.pricePaid * rec.quantity} total
                                        </p>
                                      ) : sm ? (
                                        <p className="text-[11px] text-gray-400 mt-0.5">
                                          Price: <span className="font-semibold text-gray-600">₹{sm.price}</span>
                                          <span className="text-gray-300 mx-1">·</span>
                                          Stock: <span className={`font-semibold ${sm.quantity > 10 ? "text-emerald-500" : "text-amber-500"}`}>{sm.quantity} units</span>
                                        </p>
                                      ) : null}
                                    </div>
              ...
              {unavailable.length > 0 && (
                <div className="mt-3 bg-red-50 border border-red-100 rounded-2xl px-4 py-3.5">
                  ...
                  <div className="space-y-1.5">
                    {unavailable.map((ci) => (
                      <div key={ci.medicine.id} className="flex items-center gap-2">
                        <span className="text-base">{FORM_ICONS[ci.medicine.form] || "💊"}</span>
                        <span className="text-sm font-semibold text-red-700">{ci.medicine.name}</span>
                        <span className="text-xs text-red-400">· {ci.medicine.brand}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {receivedCount > 0 && !allCollected && (
              <div
                ...
                style={{ background: "linear-gradient(135deg,#ecfdf5,#f0fdfa)", border: "1.5px solid #a7f3d0" }}
              >
                <div>
                  <p className="text-[10px] text-gray-400 uppercase tracking-wider font-bold">Spent so far</p>
                  <p className="text-xl font-black text-emerald-600">₹{totalPaid}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-semibold text-gray-500">{receivedCount} of {totalAvailable} collected</p>
                  <p className="text-xs text-gray-400 mt-0.5">{totalAvailable - receivedCount} left to go</p>
                </div>
              </div>
            )}
            ...
            {allCollected && (
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <p className="text-base font-black text-gray-900">All medicines collected!</p>
                    <p className="text-xs text-gray-500">Total paid: <span className="font-black text-emerald-600">₹{totalPaid}</span></p>
                  </div>
                </div>
                ...
                <div className="bg-white/70 rounded-xl px-3.5 py-3 mb-4 space-y-1.5">
                  {stops.map((stop) => {
                    ...
                    return (
                      <div key={stop.shop.id} className="flex items-center justify-between text-sm">
                        <span className="flex items-center gap-1.5 text-gray-600">
                          <FaStore className="w-3 h-3 text-emerald-400" />
                          {stop.shop.name}
                        </span>
                        <span className="font-bold text-gray-800">₹{stopTotal}</span>
                      </div>
                    );
                  })}
...
export default function FindPharmaciesPage() {
  return (
    <Suspense fallback={
      <Screen><Spinner color="success" size="lg" /></Screen>
    }>
